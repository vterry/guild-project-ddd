...
type Guild struct {
	...
	name      string
	createdAt time.Time
	manageBy  valueobjects.PlayerID
	vaultId   valueobjects.VaultID
	cash      int
...
func CreateGuild(guildName string, guildOwner player.PlayerID) (*Guild, error) {
...
func initializeGuild(guildName string, guildOwner *player.Player, players map[uuid.UUID]*player.Player) *Guild {
	createAt := time.Now()
	return &Guild{
		GuildID:   NewGuildID(guildName, createAt),
		name:      guildName,
		createdAt: createAt,
		manageBy:  guildOwner,
		vaultId:   vault.NewVaultID(guildName, createAt),
		treasure:  0,
		players:   players,
