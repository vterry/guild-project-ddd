...
func (m *MockCharacterRepository) Save(character Character) error {
	args := m.Called(character)
	return args.Error(0)
}

func (m *MockCharacterRepository) FindCharacterById(id CharacterID) (*Character, error) {
	args := m.Called(id)
	if args.Get(0) == nil {
...
func (m *MockCharacterRepository) Update(character Character) error {
	args := m.Called(character)
	return args.Error(0)
}
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(vs *MockVaultService, cr *MockCharacterRepository) {
				vs.On("CreateVault").Return(vault.New(uuid.New()), nil)
		...
		},
		{
			...
			class:    class.Warrior,
			setupMocks: func(vs *MockVaultService, cr *MockCharacterRepository) {
				vs.On("CreateVault").Return(vault.New(uuid.Nil), assert.AnError)
			},
			wantErr: true,
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(cr *MockCharacterRepository) {
				...
				character, _ := CreateNewCharacter("TestChar", login, class.Warrior, vaultID)
		...
		},
		{
			...
			setupMocks: func(cr *MockCharacterRepository) {
				login, _ := login.NewLogin("user123", "test@example.com")
				character, _ := CreateNewCharacter("TestChar", login, class.Warrior, vault.New(uuid.New()))
				cr.On("FindCharacterById", characterID).Return(character, nil)
			},
			wantErr: true,
		},
...
func TestTradeItem(t *testing.T) {
	originID := CreateID(uuid.New())
	destinyID := CreateID(uuid.New())
	itemID := item.New(uuid.New())
	testItem, _ := playeritem.NewPlayerItem(itemID, "Test Item", 1)
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(cr *MockCharacterRepository) {
				login, _ := login.NewLogin("user123", "test@example.com")
				originChar, _ := CreateNewCharacter("OriginChar", login, class.Warrior, vault.New(uuid.New()))
				destinyChar, _ := CreateNewCharacter("DestinyChar", login, class.Warrior, vault.New(uuid.New()))
		...
		},
		{
			...
			setupMocks: func(cr *MockCharacterRepository) {
				...
				for i := 0; i < 10; i++ {
					item, _ := playeritem.NewPlayerItem(item.New(uuid.New()), fmt.Sprintf("Item%d", i), 1)
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(cr *MockCharacterRepository) {
				...
				character.PickGold(200) // Add more than we want to deposit
				cr.On("FindCharacterById", characterID).Return(character, nil)
				cr.On("Update", mock.AnythingOfType("Character")).Return(nil)
			},
			wantErr: false,
